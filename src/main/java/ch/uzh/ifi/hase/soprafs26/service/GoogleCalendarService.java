...
public class GoogleCalendarService {
    ...
    public void syncCalendar(Long userId) {
        ...
        unavailabilityRepository.deleteByUserIdAndSource(userId, "google");
        ...
        for (Map event : events) {
            ...
            unavailability.setSource("google");
    ...
    public void syncAllGoogleCalendars() {
        List<GoogleCalendarToken> tokens = tokenRepository.findAll();
        for (GoogleCalendarToken token : tokens) {
            try {
                syncCalendar(token.getUser().getId());
            } catch (Exception e) {
                System.err.println("Sync failed for user " + token.getUser().getId() + ": " + e.getMessage());
            }
        }
    }
